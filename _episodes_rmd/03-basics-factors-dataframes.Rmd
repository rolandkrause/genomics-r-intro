...
variants <- read.csv("../data/SRR2584863_variants.csv")
```


```{r, echo = FALSE, eval = TRUE, purl = FALSE, cache=TRUE}
...
Next, under **File/Url:** click the <KBD>Browse</KBD> button and navigate to the **CDKL5.xlsx** file located at the Dropbox folder for the course.
...
**CDKL5**, and there are no other options we need to adjust. Click the
<KBD>Import</KBD> button to import the data.

Finally, let's check the first few lines of the `cdkl5` data
frame:

```{r, echo = FALSE, purl = FALSE, eval=FALSE}
## read the file so the rest of the episode works, but don't show the code
cdkl5 <-  readxl::read_xlsx("~/Dropbox/2019-12_cologne_workshop/day1/CDKL5.xlsx")
```
...
>
> **Using the `cdkl5` data frame created above, answer the following questions**
>
> A) What are the dimensions (# rows, # columns) of the data frame?
>
> B) What are categories are there in the `ClinicalSignificance` column? *hint*: treat column as factor
>
> C) How many of each of the `ClinicalSignificance` categories are there?
>
> D) What is the `Name` for the 6th observation in this data set?
>
> E) What is the median value of the variable `pos_aa`?
>
> F) Rename the column `CADD_phred` to `cadd`?
>
> G) Create a new column named `log_freq` and set it equal to the decadic log of `gnomAD_freq`?
>
> H) Save the edited `cdkl5` data frame as "exercise_solution.csv" in your current working directory.
>
>> ## Solution
>> ```{r, purl = FALSE, eval=FALSE}
>> dim(cdkl5)
>> levels(as.factor(cdkl5$ClinicalSignificance))
>> table(as.factor(cdkl5$ClinicalSignificance))
>> cdkl5[7,7]
>> median(cdkl5$genome_size)
>> colnames(cdkl5)[colnames(cdkl5) == "sample"] <- "sample_id"
>> cdkl5$log_freq <- log10(cdkl5$gnomAD_freq)
>> write.csv(cdkl5, file = "exercise_solution.csv")
>> ```
> {: .solution}
{: .challenge}
